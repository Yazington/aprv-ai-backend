...
class RagService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            await self.mongo_service.engine.get_database().command(
                "createIndex",
                "embeddings",
                {"conversation_id": 1, "source_file": 1}
            )
    ...
    async def insert_to_rag(self, conversation_id: str) -> None:
        ...
        task = await self.mongo_service.engine.find_one(
            Task, 
        ...
        grid_out = await self.mongo_service.engine.get_database().fs.files.find_one({"_id": task.generated_txt_id})
        if not grid_out:
            raise FileNotFoundError(f"File with ID {task.generated_txt_id} not found")

        document_text = grid_out["data"].decode("utf-8")
        ...
        await self.mongo_service.engine.get_database().embeddings.insert_many(documents)
    ...
    async def insert_to_rag_with_message(self, conversation_id: str, message: Message) -> None:
        ...
        for pdf_id in message.uploaded_pdf_ids:
            grid_out = await self.mongo_service.engine.get_database().fs.files.find_one({"_id": ObjectId(pdf_id)})
            if not grid_out:
        ...
        await self.mongo_service.engine.get_database().embeddings.insert_many(all_documents)
    ...
    async def search_similar(self, query: str, conversation_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = self.mongo_service.engine.get_database().embeddings.find(
                {"conversation_id": conversation_id},
